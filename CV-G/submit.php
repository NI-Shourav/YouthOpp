...
$sql = "SELECT * FROM gencv WHERE id = '$user_id'";
...
if ($row['count'] == 0) {
    $sql = "INSERT INTO gencv (id, count) VALUES ($user_id, 1)";
    $result = mysqli_query($conn, $sql);
} else {
    $sql = "UPDATE gencv SET count = count + 1 WHERE id = '$user_id'";
    $result = mysqli_query($conn, $sql);
}
...
if (Token == $_POST['token']) {
  ...
  $from1 = $_POST['from1']; // Get the from date
  $to1 = $_POST['to1']; // Get the to date
  $grade1 = $_POST['grade1']; // Get the grade


  if (strtotime($from1) === false || strtotime($to1) === false) { // Checkif the date is valid
    // alert using window.alert() function
    echo "<script>alert('Invalid date format!');</script>";
    echo "<script>window.location.href='index.php'</script>";
  }


  if (!is_numeric($grade1)) { // Check if the grade is numeric
    // alert using window.alert() function
    echo "<script>alert('Invalid grade format!');</script>";
    echo "<script>window.location.href='index.php'</script>";
  }
...
<body>
    ...
    <div class="zone-1">
      ...
      <div class="contact-box">
        ...
        <div class="call"><i class="fas fa-phone-alt"></i>
          ...
          <!-- <div class="text"><?php echo $user_id ; ?></div> -->
        </div>
        ...
        <div class="desc">
          ...
          for ($i = 0; $i < count($institutes); $i++) {
            ...
            <li>
              ...
              <div class='msg-1'>" . ucwords($degrees[$i]) . "</div>
              <div class='msg-1'>" . "" . $froms[$i] . " - " . $tos[$i] . "</div> 
              <div class='msg-1'>" . $grades[$i] . "</div>
            </li>
          </ul>";
          }
          ?>
  ...
  <div class="download">
      <button style="color: #0f102d; background-color: #f09053; padding: 10px; border-radius: 5px; 
      justify-content: center; align-items: center; display: flex; margin: 10px; font-weight: bold;
      border: none; cursor: pointer;" onclick="window.print()">Download</button>
   </div>
